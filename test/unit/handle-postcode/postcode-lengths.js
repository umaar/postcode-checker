...
test.beforeEach(async () => {
	await quibble.esm('../../../src/server/lib/validate-lsoa.js', undefined, 'bar');
});
...
test('Invalid postcodes', async t => {
	...
	t.is(await handlePostcode('&&'), 'The postcode "&&" does not look valid, please try again', 'Informs of an invalid postcode');
	t.is(await handlePostcode('%^*'), 'The postcode "%^*" does not look valid, please try again', 'Informs of an invalid postcode');
});
